"""
club_modeling.py  —  shared modelling maths (self-contained for the club app)
============================================================================
Kept dependency-free of the WC modules so this branch stands alone. Provides:

  * the analytic **Elo baseline** probability model (home / draw / away)
  * a **market baseline** helper (normalised bookmaker odds → probabilities)
  * ensemble blending by inverse-log-loss weights
  * probabilistic metrics: accuracy, multiclass log-loss, Brier score
  * calibration bins + Expected Calibration Error (ECE)
  * feature-importance normalisation for the explanation panel
...
from club_features import FEATURE_LABELS, FEATURE_ORDER
from leagues import HOME_ADV
...
def elo_effective_diff(X: np.ndarray) -> np.ndarray:
    """Elo difference including home advantage (home minus away, Elo points).

    ``elo_diff`` in the feature matrix is stored ÷100, so scale it back up.
    """
    return X[:, ELO_IDX] * 100.0 + HOME_ADV * X[:, HOME_IDX]
...
def inverse_logloss_weights(loglosses: dict[str, float]) -> dict[str, float]:
    """Ensemble weights ∝ 1/logloss (better models get more say)."""
    inv = {k: 1.0 / max(v, 1e-6) for k, v in loglosses.items()}
